...
def solver(I, V, f, c, Lx, Ly, Nx, Ny, dt, T,
    ...
    for n in It[1:-1]:
        if version == 'scalar':
            ...
            u = advance_scalar(u, u_1, u_2, f, x, y, t, n, Cx2, Cy2, dt2)
        else:
            f_a[:,:] = f(xv, yv, t[n])  # precompute, size as u
...
def advance_vectorized(u, u_1, u_2, f_a, Cx2, Cy2, dt2,
    ...
    u[1:-1,1:-1] = D1*u_1[1:-1,1:-1] - D2*u_2[1:-1,1:-1] + Cx2*u_xx + Cy2*u_yy + dt2*f_a[1:-1,1:-1]
...
def gaussian(plot_method=2, version='vectorized', save_plot=False):
